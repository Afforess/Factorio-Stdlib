...
describe('Area', function ()
    ...
    describe('Constructors', function ()
        ...
        describe('.new', function ()
            it('should error if not a table', function ()
                assert.has_error(function() A.new(1) end, 'Missing area table')
            end)
            it('should create a new version from table', function ()
                ...
                assert.same(rs(zero), rs(new_zero2))
            ...
            it('should create a new version if a copy', function ()
                local zero2 = A.new(zero, true)
                assert.not_same(rs(zero), rs(zero2))
                assert.not_same(rs(A.new(zero)), rs(A.new(zero, true)))
            end)
            it('should return itself if already an area', function ()
                assert.same(rs(zero), rs(A.new(zero)))
                assert.not_same(rs(zero), rs(A.new(raw_zero)))
            end)
            it('should have a metatable', function ()
                assert.same('area', getmetatable(A.new(zero)).__class)
                assert.same('position', getmetatable(zero.left_top).__class)
            end)
        end)
        ...
        it('.copy', function ()
            A.new(zero)
            assert.not_same(rs(zero), rs(zero:copy()))
            assert.not_same(rs(zero), rs(A.copy(zero)))
        end)
    ...
    describe('Area Methods', function ()
        ...
        it('.adjust', function ()
            assert.same(A(-1, 0, 1, 0), A(one):copy():adjust{0, -1})
            assert.same(A(0, -1, 0, 1), A(one):copy():adjust{-1, 0})
            assert.same(A(-2, 0, 2, 0), A(one):copy():adjust{1, -1})
        end)
        it('.flip', function ()
            local a1 = A{{-2, -1}, {2, 1}}
        ...
        it('.offset', function ()
            ...
            local pos = {-0.5, 2.5}
            assert.same(A(-0.5, -2.5, 2.5, -0.5), Area.offset(a1, pos))
            assert.same(A(-1,0,2,2), Area.offset(a1, pos))
            assert.same(A(2,-3,5,-1), Area.offset(orig, 2))
        end)
        it('.translate', function ()
            local a1 = A{{-12, -12}, {10, 8}}
            local orig = a1:copy()
            assert.same(A(-11, -13, 11, 7), A.translate(a1, 1, 1))
            assert.same(orig, a1:translate(5, 1))
        end)
        it('.round', function ()
    ...
    describe('Area Functions', function ()
        ...
        it('.loaded', function ()
            assert.is_false(A.loaded(one))
            assert.is_true(A(one):loaded())
        end)
        it('.pack', function ()
    ...
    describe('Area object metamethods', function ()
        ...
        it('.__call', function ()
            assert.same(Area.copy, getmetatable(one).__call)
            local one2 = one()
