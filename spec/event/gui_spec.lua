...
describe('Gui', function()
    ...
    it('.register should register a handler for a given pattern', function()
        ...
        assert.is_not_nil( Gui._registry[1] )
        assert.is_not_nil( Gui._registry[1]["test_pattern"] )
        assert.equals( 1, #Gui._registry[1]["test_pattern"] )
        assert.equals( function_a, Gui._registry[1]["test_pattern"][1] )
        assert.is_not_nil( Event._registry[1] )
        assert.equals( 1, #Event._registry[1] )
        assert.equals( Gui.dispatch, Event._registry[1][1] )
    end)
    ...
    it('.register should replace a handler when given a new one for a given pattern', function()
        ...
        assert.is_not_nil( Gui._registry[1] )
        assert.is_not_nil( Gui._registry[1]["test_pattern"] )
        assert.equals( 2, #Gui._registry[1]["test_pattern"] )
        assert.equals( function_a, table.find(Gui._registry[1]["test_pattern"], function(x) return x==function_a; end) )
        assert.equals( function_b, table.find(Gui._registry[1]["test_pattern"], function(x) return x==function_b; end) )
        assert.is_not_nil( Event._registry[1] )
        assert.equals( 1, #Event._registry[1] )
        assert.equals( Gui.dispatch, Event._registry[1][1] )
    end)
    ...
    it('.register should keep all existing patterns when a new one is registered', function()
        ...
        assert.is_not_nil( Gui._registry[1] )
        assert.is_not_nil( Gui._registry[1]["test_pattern3"] )
        assert.equals( 1, #Gui._registry[1]["test_pattern3"] )
        assert.equals( function_c, Gui._registry[1]["test_pattern3"][1] )
        assert.is_not_nil( Gui._registry[1]["test_pattern2"] )
        assert.equals( 1, #Gui._registry[1]["test_pattern2"] )
        assert.equals( function_b, Gui._registry[1]["test_pattern2"][1] )
        assert.is_not_nil( Gui._registry[1]["test_pattern"] )
        assert.equals( 1, #Gui._registry[1]["test_pattern"] )
        assert.equals( function_a, Gui._registry[1]["test_pattern"][1] )
        assert.is_not_nil( Event._registry[1] )
    ...
    it('.register should keep existing patterns when one is removed', function()
        Gui.register( 1, "test_pattern", function_a )
        Gui.register( 1, "test_pattern2", function_b )
        Gui.register( 1, "test_pattern3", function_c )
        ...
        assert.is_not_nil( Event._registry[1] )
        assert.equals( function_c, Gui._registry[1]["test_pattern3"][1] )
        assert.is_nil( Gui._registry[1]["test_pattern2"] )
        assert.equals( function_a, Gui._registry[1]["test_pattern"][1] )
    end)

    it('Gui.register should cause Event.register to register a listener', function()
        local s = spy.on(script, "on_event")
        Gui.register( 1, "test_pattern", function_a )
        assert.spy(s).was_called()
        assert.spy(s).was_called_with(1, Event.dispatch)
    end)
...
--        assert.spy(s).was_called(1) --This is failing to spy on Gui.dispatch?
    ...
    it('.dispatch should print an error to connected players if a handler throws an error', function()
        ...
        assert.is_not_nil( Gui._registry[1]["test_pattern"][1] )
    ...
    it('.remove should fail if a non-string gui_element_pattern is passed', function()
        assert.has_no.errors(function() Gui.remove( 1, nil, "test_pattern" ) end)
    ...
    it('.add should accept an array of event id values', function()
        Gui.register( {1,2,3}, "test_pattern", function_a )
        assert.equals( 3, table.count_keys(Gui._registry) )
        assert.equals( 3, table.count_keys(Event._registry) )
        assert.equals( function_a, Gui._registry[1]["test_pattern"][1] )
        assert.equals( function_a, Gui._registry[2]["test_pattern"][1] )
        assert.equals( function_a, Gui._registry[3]["test_pattern"][1] )
    end)

    it('.remove should accept an array of event id values', function()
        Gui.register( 1, "test_pattern", function_a )
        Gui.register( 2, "test_pattern", function_a )
        Gui.register( 3, "test_pattern", function_a )
        Gui.register( 4, "test_pattern", function_a )
        Gui.remove( {2, 4} )
        assert.equals( 2, table.count_keys(Gui._registry) )
        assert.equals( 2, table.count_keys(Event._registry) )
        assert.equals( function_a, Gui._registry[1]["test_pattern"][1] )
        assert.equals( Gui.dispatch, Event._registry[1][1] )
        assert.equals( function_a, Gui._registry[3]["test_pattern"][1] )
        assert.equals( Gui.dispatch, Event._registry[3][1] )
    end)
    ...
    it('.remove should remove only the handler of given pattern', function()
        ...
        assert.is_true( table.count_keys(Event._registry[1]) == 1)
        assert.is_true( table.count_keys(Gui._registry[1]) == 2)
        assert.equals( function_a, Gui._registry[1]["test_pattern"][1] )
        assert.equals( function_c, Gui._registry[1]["test_pattern3"][1] )
        assert.is_nil( Gui._registry[1]["test_pattern2"] )
    end)
