...
local Area = Position.Area
local Direction = require('__stdlib__/util/direction')
local Orientation = require('__stdlib__/util/orientation')

local string = require('__stdlib__/util/string')
local math = require('__stdlib__/util/math')
...
-- @usage Position.new({x = 0, y = 0}) -- returns {x = 0, y = 0}
-- @tparam Concepts.Position pos the position table or array to create a new position for
-- @treturn Concepts.Position
function Position.new(pos)
    return setmetatable({x = pos.x or pos[1], y = pos.y or pos[2]}, metatable)
end
...
function Position.from_string(pos_string)
    local ret, ret2 = load('return ' .. pos_string)()
    local type = type(ret)
    if type == 'table' then return new(ret.x or ret[1], ret.y or ret[2]) end
    return new(ret, ret2)
end
...
--- Create a new incremented position each time it is called.
...
-- @tparam[opt=false] boolean should the position be mutated or created new.
-- @treturn function @{increment_closure} a function closure that returns a new incremented position
function Position.increment(pos, inc_x, inc_y, increment_initial, mutate)
