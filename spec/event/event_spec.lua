...
describe('Event', function ()
    ...
    insulate('.register', function()
        it('should register itself with factorio when the initial listener \z
            ...
            World.bootstrap()
            local Event = require('__stdlib__/stdlib/event/event')
            local spy_on_init = spy.on(Event._script, 'on_init')
            local spy_on_load = spy.on(Event._script, 'on_load')
            local spy_on_configuration_changed = spy.on(Event._script, 'on_configuration_changed')
