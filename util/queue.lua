...
local Queue = {__class = 'Queue'}
local Metatable = {__index = require('__stdlib__/stdlib')}
Metatable.__index.Queue = Queue
setmetatable(Queue, Metatable)

local table = require('__stdlib__/util/table')
...
function Metatable.__call(_, ...)
    ...
    for _, push in pairs({...}) do queue(push) end
    return queue
end
...
function Queue.load(queue)
    if type(queue) == 'table' and queue.first then return setmetatable(queue, meta) end
end
...
function Queue.wrapper(self, func_name, ...)
    if Queue[func_name] then Queue[func_name](self, ...) end
    return self
end
...
function Queue.pop_first(queue)
    if Queue.is_empty(queue) then return nil end
...
local function remove(queue, index)
    ...
    if ret ~= nil then
        for i = index + 1, queue.last do queue.objects[i - 1] = queue.objects[i] end
...
function Queue.find(queue, find)
    for i, v in pairs(queue) do if v == find then return i end end
end
...
function Queue.sort(queue, func)
    local sorted = {}
    for _, v in pairs(queue) do if v ~= nil then sorted[#sorted + 1] = v end end
...
function Queue.next(queue, index, pop)
    ...
    for i = index, queue.last do
        ...
        if v ~= nil then return i, pop and Queue.pop_at(queue, i) or v end
    end
    return nil, nil
end
...
do
    ...
    meta.__index = function(self, k)
        if type(k) == 'number' then
            ...
            if v == nil then return Queue[k] end
            return v
        end
    end
    ...
    meta.__add = function(queue1, queue2)
        ...
        if lhs then
            for _, v in pairs(queue1.objects) do new:push(v) end
        else
            new:push(queue1)
        end
        if rhs then
            for _, v in pairs(queue2.objects) do new:push(v) end
        else
            new:push(queue2)
        end
        return new
