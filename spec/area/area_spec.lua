...
describe('Area Spec', function()
    it('should validate whether positions are inside of an area', function()
        ...
        assert.truthy(Area.inside_area(pos, area))

        local area = {left_top = {100, 100}, right_bottom = {95, 95}}
        assert.falsy(Area.inside_area(pos, area))
    end)
    ...
    it('should validate area offsets', function()
        ...
        local pos = {-0.5, 2.5}
        assert.same({x = -0.5, y = -2.5}, Area.offset_area(area, pos).left_top)
        assert.same({x = 2.5, y = -0.5}, Area.offset_area(area, pos).right_bottom)
    end)
